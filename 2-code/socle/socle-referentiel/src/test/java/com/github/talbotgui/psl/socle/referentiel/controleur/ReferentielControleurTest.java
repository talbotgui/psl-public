...
/**
 * Classe de test spécifique au ReferentieleClient se basant sur les cas de test décrits dans la classe AbstractReferentielControleurTest
 */
//Configuration du test
@SpringBootTest(classes = SocleReferentielApplication.class, webEnvironment = WebEnvironment.RANDOM_PORT)
class ReferentielControleurTest extends AbstractReferentielControleurTest {
	...
	protected void avantChaqueTest(TestInfo testInfo) {
		...
		this.leClientAutiliser = new ReferentielClient(null, "http://localhost:" + this.port, false);
		((ReferentielClient) this.leClientAutiliser).enregistrerTokenJwtAutiliser(super.tokenJwtValableDurantLeTest);
	}
	...
	protected void test00Securite() {
		...
		((ReferentielClient) this.leClientAutiliser).enregistrerTokenJwtAutiliser(tokenInvalide);

...
class HubeeClientTest extends AbstractTest {

	@Autowired
	private HubeeClient client;

	@Test
	void testReferentielDesAbonnesActifsHubee() {
		//
		//
		Map<String, List<AbonneHubeeDto>> abonnes = this.client.telechargerLeReferentielDesAbonnesActifsHubee();
		//
		Assertions.assertNotNull(abonnes);
		int nbAbonnements = abonnes.values().stream().map(l -> l == null ? 0 : l.size()).reduce(0, Integer::sum);
		Assertions.assertTrue(nbAbonnements > 30);
	}

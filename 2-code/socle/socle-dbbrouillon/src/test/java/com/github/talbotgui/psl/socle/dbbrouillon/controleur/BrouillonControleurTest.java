...
@SpringBootTest(classes = SocleDbbrouillonApplication.class, webEnvironment = WebEnvironment.RANDOM_PORT,
		// pour ne pas déclencher d'initialisation des données au démarrage du contexte
		properties = { "jdd.initialisation=false" })
class BrouillonControleurTest extends AbstractBrouillonControleurTest {
	...
	protected void avantChaqueTest(TestInfo testInfo) {
		...
		this.leClientAutiliser = new DbbrouillonClient(null, "http://localhost:" + this.port, false);
		((DbbrouillonClient) this.leClientAutiliser).enregistrerTokenJwtAutiliser(this.tokenJwtValableDurantLeTest);
	}
	...
	protected void test00Securite() {
		...
		((DbbrouillonClient) this.leClientAutiliser).enregistrerTokenJwtAutiliser(tokenInvalide);

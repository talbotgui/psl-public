...
@SpringBootTest(classes = SocleReferentielExterneApplication.class, webEnvironment = WebEnvironment.RANDOM_PORT)
class ReferentielExterneControleurTest extends AbstractReferentielExterneControleurTest {
	...
	protected void avantChaqueTest(TestInfo testInfo) {
		...
		super.leClientAutiliser = new ReferentielExterneClient(null, "http://localhost:" + this.port, false);
		((ReferentielExterneClient) this.leClientAutiliser).enregistrerTokenJwtAutiliser(super.tokenJwtValableDurantLeTest);
	}
	...
	protected void test00Securite() {
		...
		((ReferentielExterneClient) this.leClientAutiliser).enregistrerTokenJwtAutiliser(tokenInvalide);
